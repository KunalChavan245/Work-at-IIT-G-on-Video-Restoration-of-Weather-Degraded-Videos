...
def parse_args():
    ...
    parser.add_argument('--data_name', type=str, default='Thick', choices=['unified', 'Snow', 'underwater'])
...
class RainDataset(Dataset):
    def __init__(self, data_path, data_path_test, data_name, data_type, patch_size=None, length=None):
        ...
        # self.rain_images = sorted(glob.glob('{}/{}/{}/inp/*.png'.fomat(data_path, data_name, data_type)))
...
#     transforms.Resize((256, 256)),  # Resize frames to 256x256
#     T.to_tensor(),
# ])
    ...
    def __getitem__(self, idx):
        ...
        if self.data_type == 'train':
            for i in range(len(frame_files)):
                ...
                frame1_ip = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(transforms.Resize((256, 256))(T.to_tensor(frame1[:,frame1.shape[1]//2:,:])))
                frame1_gt = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(transforms.Resize((256, 256))(T.to_tensor(frame1[:,:frame1.shape[1]//2,:])))
                if torch.rand(1) < 0.5:
